 ...
 * define mail specific properties (see java mail for more info) in the fax4j properties.<br>
 * Implementing SPI class will have to implement the createXXXFaxJobMessage methods.<br>
 * These methods will return the message to be sent for that fax job operation. In case the method returns null, this
 * class will throw an UnsupportedOperationException exception. <br>
 ...
 * <td>True to reuse the same mail connection for all fax activites, false to create a new mail connection for each fax
 * activity.</td>
 * <td>false</td>
 * <td>false</td>
 * <td>false</td>
 ...
 * <b>Limitations:</b><br>
 * <ul>
 * <li>This SPI is based on the java mail infrastructure, therefore this SPI cannot connect to mail servers through a
 * proxy server.
...
public abstract class AbstractMailFaxClientSpi extends AbstractFax4JClientSpi {
    /**
     * The use persistent connection flag which defines if the SPI will use a new connection for each request or will
    ...
    public enum FaxClientSpiConfigurationConstants {
        /**
         * The use persistent connection property key which defines if the SPI will use a new connection for each
         * request or will reuse the same connection
         */
    ...
    protected void initializeImpl() {
        ...
        this.usePersistentConnection = Boolean.parseBoolean(
                this.getConfigurationValue(FaxClientSpiConfigurationConstants.USE_PERSISTENT_CONNECTION_PROPERTY_KEY));
        logger.logDebug(new Object[] { "Using persistent connection: ", String.valueOf(this.usePersistentConnection) },
                null);

        // setup connection factory
        String className = this
                .getConfigurationValue(FaxClientSpiConfigurationConstants.CONNECTION_FACTORY_CLASS_NAME_PROPERTY_KEY);
