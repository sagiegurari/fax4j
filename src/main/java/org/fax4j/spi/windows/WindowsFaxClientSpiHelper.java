        ...
        synchronized (WindowsFaxClientSpiHelper.NATIVE_LOCK) {
            if (!WindowsFaxClientSpiHelper.nativeLibraryLoaded) {
                try {
                    ...
                    logger.logError(new Object[] { "Error while loading native library from runtime path." },
                            throwable);
                }
                ...
                if (!WindowsFaxClientSpiHelper.nativeLibraryLoaded) {
                    try {
                        ...
                        logger.logError(new Object[] { "Error while loading native library from native path." },
                                throwable);
                    }
                }
            }
