 ...
 * interface for those using this product.<br>
 * <br>
...
public class PhaxioFaxClientSpi extends AbstractMultiPart2JSONHTTPFaxClientSpi {
    ...
    protected void updateFaxJob2HTTPRequestConverterConfiguration(Map<String, String> configuration) {
        ...
        configuration.put(MessageFormat.format(
                ...
                propertyPart), "id");
        configuration.put(MessageFormat.format(
                FaxJob2HTTPRequestConverterConfigurationConstants.ADDITIONAL_PARAMETER_PROPERTY_KEY_PREFIX.toString()
                        + "api_key",
                propertyPart), apiKey);
        configuration.put(MessageFormat.format(
                FaxJob2HTTPRequestConverterConfigurationConstants.ADDITIONAL_PARAMETER_PROPERTY_KEY_PREFIX.toString()
                        + "api_secret",
                propertyPart), apiSecret);
    }
    ...
    protected void updateHTTPResponseHandlerConfiguration(Map<String, String> configuration) {
        ...
        // modify configuration
        configuration.put(MessageFormat.format(
                JSONHTTPResponseHandlerConfigurationConstants.SUBMIT_JSON_OUTPUT_PROPERTY_KEY.toString(), propertyPart),
                "faxId");
        configuration.put(MessageFormat.format(
                JSONHTTPResponseHandlerConfigurationConstants.ERROR_DETECTION_PATH_PROPERTY_KEY.toString(),
                propertyPart), "success");
        configuration.put(MessageFormat.format(
                JSONHTTPResponseHandlerConfigurationConstants.ERROR_DETECTION_VALUE_PROPERTY_KEY.toString(),
                propertyPart), "false");
        configuration.put(MessageFormat.format(
                JSONHTTPResponseHandlerConfigurationConstants.ERROR_MESSAGE_PATH_PROPERTY_KEY.toString(), propertyPart),
                "message");
        configuration.put(MessageFormat.format(
                JSONHTTPResponseHandlerConfigurationConstants.GET_STATUS_JSON_OUTPUT_PROPERTY_KEY.toString(),
                propertyPart), "data;status");
        configuration.put(MessageFormat.format(
                JSONHTTPResponseHandlerConfigurationConstants.PENDING_STATUS_MAPPING_PROPERTY_KEY.toString(),
                propertyPart), "queued;pendingbatch");
        configuration.put(MessageFormat.format(
                JSONHTTPResponseHandlerConfigurationConstants.IN_PROGRESS_STATUS_MAPPING_PROPERTY_KEY.toString(),
                propertyPart), "inprogress");
        configuration.put(MessageFormat.format(
                JSONHTTPResponseHandlerConfigurationConstants.ERROR_STATUS_MAPPING_PROPERTY_KEY.toString(),
                propertyPart), "failure;partialsuccess");
    }
