 ...
 * availability of executables or validation that a certain fax4j property is defined and contains a value.<br>
 * Once all conditions are met the internal SPI is created, otherwise the next SPI configuration is used.<br>
 ...
 * This SPI will go over the list one by one based on the configured order and will validate the type conditions.<br>
 * Once an SPI is found for which all conditions are met, the adapter SPI will create an instance of it and use it to
 * proxy all fax operations to.</td>
 ...
 * <td>For each type defined in the org.fax4j.spi.adapter.internal.spi.types property, you can define custom conditions
 * to validate.<br>
 ...
 * (for example: org.fax4j.spi.adapter.stable=true)<br>
 ...
 * <li>java-class:xxx where xxx is the SPI class name based on the fax4j mapping (for example:
 * org.fax4j.spi.type.map.adapter=org.fax4j.spi.adapter.AdapterFaxClientSpi)
 * <li>stable
 * </ul>
 * </td>
 * <td>org.fax4j.spi.adapter.internal.spi.condition.windows=OS:windows;native-lib:winfax<br>
 * org.fax4j.spi.adapter.internal.spi.condition.vbs=OS:windows;executable:cscript.exe<br>
 ...
 * example:<br>
 * org.fax4j.spi.adapter.configuration.override.<b>org.fax4j.proxy.enabled</b>=false overrides the
 * org.fax4j.proxy.enabled property for the interal SPI.</td>
 * <td>org.fax4j.spi.adapter.configuration.override.org.fax4j.proxy.enabled=false</td>
 * <td>none</td>
 * <td>false</td>
 * </tr>
 * </table>
...
public class AdapterFaxClientSpi extends AbstractAdapterFaxClientSpi {
    ...
    public enum FaxClientSpiConfigurationConstants {
        ...
        /**
         * The stable condition - checks for a fax4j property org.fax4j.spi.xxx.stable is defined and equals true, where
         * xxx is the SPI key (for example: org.fax4j.spi.adapter.stable=true)
         */
    ...
    protected void initializeChildFaxClientSpi() {
        ...
        logger.logDebug(new Object[] { FaxClientSpiConfigurationConstants.SPI_TYPES_PROPERTY_KEY, " value is ", types },
                null);

        if (types != null) {
            // split type values
            String[] typeValues = types
                    .split(FaxClientSpiConfigurationConstants.PROPERTY_VALUE_LIST_SEPARATOR.toString());
            ...
            for (int index = 0; index < amount; index++) {
                ...
                conditionString = this.getConfigurationValue(
                        FaxClientSpiConfigurationConstants.SPI_CONDITION_PROPERTY_KEY_PREFIX + typeValue);
                logger.logDebug(
                        new Object[] { "Configured condition for type: ", typeValue, " is set to: ", conditionString },
                        null);
                ...
                conditionStringBuffer.append(FaxClientSpiConfigurationConstants.PROPERTY_BLOCK_VALUE_SEPARATOR);
                conditionStringBuffer.append(this.getConfigurationValue(
                        FaxClientSpiFactory.SPI_CLASS_NAME_TYPE_MAP_PROPERTY_KEY_PREFIX + typeValue));
                conditionStringBuffer.append(FaxClientSpiConfigurationConstants.PROPERTY_VALUE_LIST_SEPARATOR);
                ...
                logger.logDebug(
                        new Object[] { "Updated condition for type: ", typeValue, " is set to: ", conditionString },
                        null);

                // split condition to blocks
                conditionBlocks = conditionString
                        .split(FaxClientSpiConfigurationConstants.PROPERTY_VALUE_LIST_SEPARATOR.toString());
                ...
                for (int blockIndex = 0; blockIndex < subAmount; blockIndex++) {
                    ...
                    conditionBlockValues = conditionBlock
                            .split(FaxClientSpiConfigurationConstants.PROPERTY_BLOCK_VALUE_SEPARATOR.toString());

                    // check condition block contains only 2 parts (key and value)
                    if (conditionBlockValues.length != 2) {
                        throw new FaxException("Condition block must contain 2 parts separated by "
                                + FaxClientSpiConfigurationConstants.PROPERTY_BLOCK_VALUE_SEPARATOR);
                    }
                    ...
                    // validate condition
                    logger.logDebug(new Object[] { "Validating condition for type: ", typeValue, " key: ", conditionKey,
     ...
     *            native-lib - checks that native lib can be loaded<br>
     *            executable - checks executable is on system path<br>
     *            stable - checks for a fax4j property org.fax4j.spi.xxx.stable is defined and equals true, where xxx is
     *            the SPI key (for example: org.fax4j.spi.adapter.stable=true)
     * @param value
     *            The condition value
    ...
    protected boolean validateCondition(String key, String value) {
        ...
        case STABLE_CONDITION:
            // get property key
            String propertyKey = FaxClientSpiConfigurationConstants.SPI_STABLE_PROPERTY_KEY_PREFIX + value
                    + FaxClientSpiConfigurationConstants.SPI_STABLE_PROPERTY_KEY_SUFFIX;
    ...
    protected Properties createFaxClientSpiConfiguration() {
        ...
        String overridePrefix = FaxClientSpiConfigurationConstants.SPI_CONFIGURATION_OVERRIDE_PROPERTY_KEY_PREFIX
                .toString();
