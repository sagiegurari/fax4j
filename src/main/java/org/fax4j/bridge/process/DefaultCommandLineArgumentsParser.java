 ...
 * info from the command line arguments.<br>
 ...
 * @since 0.40.6
 */
public class DefaultCommandLineArgumentsParser extends AbstractRequestParser<String[]>
    ...
    protected FileInfo getFileInfoFromInputDataImpl(String[] inputData) {
        ...
        if ((amount % 2) == 0) {
            ...
            for (int index = 0; index < inputData.length; index = index + 2) {
                ...
                if (argument.equals("-file")) // only looks for file argument, the update fax job will validate the full
                                              // command line arguments for correctness
                {
                    // get file
