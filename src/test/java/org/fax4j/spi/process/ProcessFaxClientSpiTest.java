...
public class ProcessFaxClientSpiTest {
    ...
    public void setUp() throws Exception {
        ...
        String templateText = "target.address=${target.address} " + "target.name=${target.name} "
                + "sender.name=${sender.name} " + "sender.fax.number=${sender.fax.number} "
                + "sender.email=${sender.email} " + "file: ${file}";
        configuration.setProperty("org.fax4j.spi.process.submit.template.command",
                commandPrefix + "submit " + templateText);
        configuration.setProperty("org.fax4j.spi.process.suspend.template.command",
                commandPrefix + "suspend " + templateText);
        configuration.setProperty("org.fax4j.spi.process.resume.template.command",
                commandPrefix + "resume " + templateText);
        configuration.setProperty("org.fax4j.spi.process.cancel.template.command",
                commandPrefix + "cancel " + templateText);
        configuration.setProperty("org.fax4j.spi.process.get.status.template.command",
                commandPrefix + "get.status " + templateText);
        configuration.setProperty("org.fax4j.spi.process.output.validator",
                ExitCodeProcessOutputValidator.class.getName());
        ...
        this.faxClientSpi = (ProcessFaxClientSpi) TestUtil.createFaxClientSpi(ProcessFaxClientSpi.class.getName(),
                configuration);
    ...
    public void executeProcessTest() throws Exception {
        ...
        ProcessOutput processOutput = this.faxClientSpi.executeProcess(this.faxJob, command,
